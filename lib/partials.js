...
module.exports = function(params){
	...
	var parseFile = function(file, templateRoot, filesMap, delimiter, commentDelimiter){
		...
		_.each(partials, function(partial, fileIndex){
			...
			while(filePosition < fileLines.length){
				...
				if(fileLines[filePosition] == next){
					...
					if(fileLines[filePosition].length && !fileLines[filePosition].match(new RegExp('^' + commentDelimiter + '*(.*)$'))){
